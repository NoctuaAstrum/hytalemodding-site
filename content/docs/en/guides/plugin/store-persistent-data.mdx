...
Creating a custom component class leverages the Component system made with the ECS architecture in mind. For more information on Components, check out the [ECS Guide on Components](../ecs/hytale-ecs-theory#components).
...
public class YourPlayerData implements Component<EntityStore> {
    ...
    public static ComponentType<EntityStore, YourPlayerData> getComponentType() {
        return YourPlugin.instance.getYourPlayerDataComponent();
    }
}
```
...
private void someEntryMethod(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store)
{
     ...
     if(store.getComponent(ref, YourPlayerData.getComponentType()) != null)
     {
          // here we implement logic that updates the component
         var comp = store.getComponent(ref, YourPlayerData.getComponentType())
         // declare public fields or methods in the component class
         comp.SomeString = "An Updated Field";
         // replace the component with the new updated one
         store.replaceComponent(ref, YourPlayerData.getComponentType(), comp);
     } else {
          ...
          store.putComponent(ref, YourPlayerData.getComponentType(), myCustomComp);
     }
}
```
...
public class IncompleteCustomCommand extends AbstractPlayerCommand {
    ...
    protected void execute(
        ...
        YourPlayerData customData = store.ensureAndGetComponent(ref, YourPlayerData.getComponentType());
